...
class RAGEngine:
    ...
    def _create_prompt(self, query: str, chunks: List[Dict[str, Any]]) -> str:
        ...
        # Use Llama 3.1 instruction format for better results
...
"""
        ...
        try:
            ...
            try:
                for step in range(max_tokens):
                    ...
                    # Use greedy decoding for better quality (temperature = 0)
                    if temperature == 0:
                        ...
                        k = 20  # Reduced top-k for more focused sampling
                        top_k_indices = np.argsort(last_token_logits)[-k:]
                    ...
                    try:
                        ...
                        if token_text and len(token_text.strip()) > 0:
                            # Skip tokens that are just punctuation or special characters
                            if len(token_text.strip()) == 1 and token_text.strip() in '.,:;!?()[]{}"\'':
                                continue
                    ...
                    if len(generated_text) > 20:
                        ...
                        if last_20_chars.count('the') > 5 or last_20_chars.count('a') > 5:
    ...
    def query(self, question: str, k: int = 5, max_tokens: int = 256, temperature: float = 0.0, stream: bool = True) -> str:
        """
        ...
        try:
            ...
            # Use simple generation for better compatibility
            response_generator = self._generate_response_simple(prompt, max_tokens, temperature)
            full_response = ""
            ...
            if len(full_response.strip()) < 5:
                console.print(f"\nWarning: Generated response is too short: '{full_response}'", style="yellow")
                full_response = "I apologize, but I couldn't generate a proper response based on the available context."
            elif full_response.count('the') > len(full_response) * 0.3:
                console.print(f"\nWarning: Generated response contains too many repetitive words", style="yellow")
                full_response = "I apologize, but I couldn't generate a coherent response based on the available context."
